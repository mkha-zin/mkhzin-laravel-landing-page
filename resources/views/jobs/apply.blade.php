        ...
        <div class="form-card">
            <form method="POST" enctype="multipart/form-data" novalidate>
                ...
                <div class="row g-3">
                    ...
                    @if($job->resume_state && ($job->resume_state == 'required' || $job->resume_state == 'optional'))
                        <!-- Resume -->
                        <div class="col-12">
                            ...
                            <input type="file" name="resume" style="background-color: transparent; color: black"
                                   ...
                                   accept=".pdf,.doc,.docx" {{ $job->resume_state == 'required' ? 'required' : '' }}>
                            @error('resume')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        @foreach ($job->questions as $index => $question)
                            <div class="{{ count($job->questions) > 1 ? 'col-md-6' : 'col-12' }}">
                                ...
                                @if ($question['type'] === 'text')
                                    <input required style="background-color: transparent; color: black" type="text"
                                ...
                                @elseif ($question['type'] === 'yes_no')
                                    <select required style="background-color: transparent; color: black"
                                            name="questions[{{ $index }}][answer]"
                                ...
                                @elseif ($question['type'] === 'number')
                                    <input required style="background-color: transparent; color: black" type="number"
                                           name="questions[{{ $index }}][answer]"
                                           class="form-control @error("questions.$index.answer") is-invalid @enderror"
                                           value="{{ old("questions.$index.answer") }}">
                                    @error("questions.$index.answer")
                ...
                <div class="text-{{  $dir == 'ltr' ? 'right' : 'left' }} mt-4">
                    <button type="submit"
                            class="btn btn-primary1 text-white">{{ __('home.Submit Application') }}</button>
                </div>
            </form>
        </div>
    </div>
